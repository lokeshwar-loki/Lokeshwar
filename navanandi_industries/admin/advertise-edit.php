<?php
    ...
    if(isset($_GET['delete']) && is_numeric($_GET['delete'])){
        ...
        mysqli_query($connection, "DELETE FROM `global_advertize` WHERE `id` = '$adId'");
    }
	...
	if(isset($_GET['id'])){
        ...
        $usefulNumbersStatusSQL = "SELECT * FROM global_advertize WHERE id = '$id'";
        ...
        if(mysqli_num_rows($usefulNumbersStatusResult) == 1){
            ...
            if($usefulNumbersStatusRow['status'] == 1){
                $usefulNumbersStatusUpdateSQL = "UPDATE global_advertize SET status = 0 WHERE id = '$id'";
            }else{
                $usefulNumbersStatusUpdateSQL = "UPDATE global_advertize SET status = 1 WHERE id = '$id'";
            }
